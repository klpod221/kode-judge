...
from app.db_utils import get_db_session, update_submission_status, update_submission_result
        ...
        try:
            ...
            if language.get("compile_command"):
                ...
                if compile_result.returncode != 0:
                    ...
                    compile_meta_dict = dict(line.strip().split(":") for line in compile_meta_data.strip().split("\n"))
                    ...
                    return {"result": "compile_error", "stdout": compile_stdout, "stderr": compile_stderr, "meta": compile_meta_dict}
            ...
            if language["name"].lower() == "node.js":
                stderr_txt = stderr_txt.replace("Warning: disabling flag --expose_wasm due to conflicting flags\n", "")

            meta_dict = dict(line.strip().split(":") for line in meta_data.strip().split("\n"))
            ...
            return {"result": "success", "stdout": stdout_txt, "stderr": stderr_txt, "meta": meta_dict}
            ...
            # Cleanup the sandbox
            if box_path:
                cleanup_cmd = [ISOLATE_BINARY, f"--box-id={os.path.basename(box_path_str)}", "--cleanup"]
