...
from app.schemas.submission import SubmissionCreate, SubmissionRead, SubmissionID
...
submission_queue = Queue(settings.REDIS_PREFIX + "_submission_queue", connection=redis_conn)

async def get_db() -> AsyncSession:
    async with AsyncSessionLocal() as session:
        yield session

@router.post("/", response_model=None, summary="Create a new code submission")
async def create_submission(submission: SubmissionCreate, wait: bool = False, db: AsyncSession = Depends(get_db)):
    # Find the language
    ...
    submission_data = SubmissionRead.model_validate(db_submission).model_dump(mode='json')
    submission_queue.enqueue(
        "app.worker.process_submission", submission_data
    )
